...
```{r Spread Graph, echo=FALSE, warning=FALSE}
...
```{r Equity Curve, echo=FALSE, warning=FALSE}
...
```{r Pair Rotation Time Line, echo=FALSE, warning=FALSE}
...
```{r Weight Sensitivity Analysis, echo=FALSE, warning=FALSE, results='hide'}
# Filter for L = 1:4 
...
```{r performance analysis, echo=FALSE, warning=FALSE}
...
```{r transaction cost optimization, echo=FALSE, warning=FALSE}
...
This ensures that all capital is deployed with no borrowing, serving as our baseline setup.
...
+ Even at leverage levels of 2 or 3 (L = 2 or L = 3), the strategy consistently outperforms SPY. This indicates that achieving a strong advantage does not require excessive leverage, demo
...
chart.Drawdown(strategy_returns, main = "Strategy Drawdown", geometric = TRUE, plot = FALSE)
