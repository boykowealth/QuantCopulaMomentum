...
measure_momentum <- function(data, lookback = NULL) {
  ...
  data <- utils::tail(data, lookback)
  
  arima_model <- forecast::auto.arima(data, d = 1, allowdrift = TRUE)  
  coefs <- stats::coef(arima_model)
  
  drift <- ifelse("drift" %in% names(coefs), coefs["drift"], 0)
  ar_terms <- sum(coefs[grep("ar", names(coefs))], na.rm = TRUE)
  
  
  momentum_magnitude <- abs(drift) + abs(ar_terms)
...
momentum_ts <- return_data %>% 
  tidyr::pivot_longer(cols = (tickers)) %>% 
  dplyr::group_by(name)
  dplyr::mutate(momentum = purrr::map_dbl(value, ~measure_momentum(.x, lookback = 20)))
